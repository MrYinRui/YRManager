...
@interface HMCheckInLogic () <HTTPAPIDelegate>
...
@property (nonatomic, assign) BOOL isChangeRoom;//!<是否换房后入住

@end

@implementation HMCheckInLogic

+ (instancetype)logicWithOrder:(HMOrder *)order vc:(UIViewController *)vc
{
    HMCheckInLogic *logic = [[HMCheckInLogic alloc] init];
...
- (void)executeAfterChangeRoom {
    ...
    self.isChangeRoom = YES;
    [self executeWithFreshOrder];
}
...
- (void)fetchData:(id)data fromServerSuccessedWithAPI:(HTTPAPI *)api
{
    ...
    {
        if (self.isChangeRoom) {// 换房后入住不再做后面的判断
            int result = [data[@"isIn"] intValue];
            ...
            if (1 == result)
            {
                ...
                [successLogic showSuccessViewWithBleSendStatus:[data[@"sendSuccess"] boolValue]];
            }
        ...
        // Noshow
        if ([data[@"isNoshow"] boolValue])
        {
            [self handleNoshowOrder:(id)[HMOrder mj_objectWithKeyValues:data[@"overdueOrderMap"][@"noshowOrder"]]];
        }
        //  逾期未离
        else if ([data[@"isOverdue"] boolValue])
        {
            [self handleOverdueOrder:(id)[HMOrder mj_objectWithKeyValues:data[@"overdueOrderMap"][@"overdueOrder"]]];
        }
        // isSameRoomOrder 是否存在不同类型的已入住订单   1.有   0.无
        else if ([data[@"isSameRoomOrder"] boolValue])
        {
            [self handleSameRoomOrder];
        }
        // 欠款
        else if ([data[@"isBalance"] boolValue] && !self.checkinAPI.ignorePay)
        {
            [self showNeedPayView];
        }
        // 入住人信息未完善
        else if ([data[@"isComplete"] boolValue] && !self.checkinAPI.noMessageCheckIn)
        {
            _tmpLodger = [HMLodger mj_objectWithKeyValues:data[@"rzrMap"][@"uncompleteRzr"]];
            [self showNobodyRecordInfo];
        }
        // 入住人信息重复 1.身份证重复 2.手机 3.身份证&手机 0
        else if ([data[@"isRepeat"] boolValue])
        {
            int type = [data[@"isRepeat"] intValue];
            
            _tmpLodger = [HMLodger mj_objectWithKeyValues:data[@"rzrMap"][@"repeatRzr"]];
            ...
            if (1 == type)
            {
                ...
                HMAlertView *view = [HMAlertView alertViewWithIconType:IconWarn msg:[NSString stringWithFormat:@"入住人：%@，身份重复，无法办理入住，是否重新添加入住人信息？？", _tmpLodger.name] rightBtnBlock:^(UIButton *rightBtn) {
                    [rightBtn setTitle:@"办理入住" forState:UIControlStateNormal];
        ...
        // 脏房
        else if ([data[@"isRoomClean"] boolValue] && !self.checkinAPI.ignoreUnclean)
        {
